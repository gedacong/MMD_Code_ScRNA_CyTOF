...
#2020-11-11  draw_expr_heatmaps添加导出abundance csv
...
#'@param scale_by_columns     逻辑变量，决定是否对各列进归一化
...
draw_expr_heatmaps<-function(xdata,
  ...
  if (!is.null(cluster_label_df)){
     xdata<-dplyr::left_join(xdata,cluster_label_df,by=colnames(cluster_label_df)[1])
  }
  ...
  copy_para_csv(output_dir,c("cluster_expr_heatmap.csv","cluster_expr_density.csv","cluster_heatmap_instruction.pptx"))
  paralist_h<-get_para(output_dir,"cluster_expr_heatmap.csv")
  eval(parse(text =paralist_h$items))
  paralist_d<-get_para(output_dir,"cluster_expr_density.csv")
...
if(trans_method=="raw") trans_fun=notrans                           
...
if(trans_method=="simpleAsinhX2") trans_fun=simpleAsinhX2
  ...
  if(is.null(groups_to_show)){
              groups_to_show=as.character(unique(groups[,major_cond,drop=T])  )
            } 
  ...
  if(trans_method %in% c("0_to_Max","simpleAsinh_0_to_Max")){
    ...
    heatmap_data_raw<-cluster_data_raw[,c(cluster_name,heatmap_ID)]
 
    write.csv(heatmap_data,file=paste0("./",output_dir,"/","heatmap_trans_data.csv"))  
    write.csv(heatmap_data_raw,file=paste0("./",output_dir,"/","heatmap_raw_data.csv"))  
  
   if (scale_by_columns==T){
        heatmap_data[,-1]<-as.data.frame(scale(heatmap_data[,-1],center = T, scale=T))
        str(heatmap_data)
   }
      ...
      #this_group="merge"  
              ...
              #生成boxplot
                ...
                gheatmap = ggplot(this_melt_htdata, aes_string(x="markers", y="clusters", fill="expression"))+
                            theme(plot.margin=unit(c(0,0,0,0), unit = "cm"))+
                            geom_tile(color=rect_border_color_h, size=rect_border_size_h)+
                ...
                eval(parse(text = paste0("gheatmap<-gheatmap+",paralist_h$themes)))
                ...
                if(!exists("width_ratio_h")) width_ratio_h<-c(5/nmarkers,1,6/nmarkers,4/nmarkers)
                if(!exists("height_ratio_h")) height_ratio_h<-c(5/nclusters,1)
                # install.packages("egg")
                library(egg)
                pdf(file=paste0("./",output_dir,"/",this_group,"_heatmap.pdf"),width=plot_width*zoom_fig_width_h,height=plot_height*zoom_fig_height_h)
                
                if(show_abundance_barplot){
                        write.csv(cluster_abandance_stat,file=paste0("./",output_dir,"/",this_group,"_abundance.csv"),row.names = F)
                  
                        plot2<-ggarrange(gblank,ddplot_marker,gblank,gblank,
                                         ddplot_cluster,gheatmap2,cluster_abundance_barplot,gheatmap_legend,
                                         ncol = 4,
                                         widths = width_ratio_h,
                                         heights = height_ratio_h)
                }else{
                        plot2<-ggarrange(gblank,ddplot_marker,gblank,
                                         ddplot_cluster,gheatmap2,gheatmap_legend,
                                         ncol = 3,
                                         widths = width_ratio_h[1,2,4],
                                         heights =height_ratio_h) 
            ...
            if(output_density_plot){
              ...
              this_melt_htdata[,"clusters"]<-factor(this_melt_htdata[,"clusters"],levels =rev(levels(this_melt_htdata[,"clusters"])),ordered = TRUE )
               ...
               if(!exists("width_ratio_d")) width_ratio_d<-c(3/nmarkers,1,0.5/nmarkers,3/nmarkers)
               if(!exists("height_ratio_d")) height_ratio_d<-c(2.5/nclusters,1,2/nclusters)               
               ...
               plot3<-ggarrange(gblank,ddplot_marker,gblank,gblank,
                                ...
                                widths =width_ratio_d,
                                heights = height_ratio_d)
               dev.off()
               pdf(file=paste0("./",output_dir,"/",this_group,"_Density_plots.pdf"),width = col_plot*zoom_fig_width_d,height = row_plot*zoom_fig_height_d)
               multiplot(plot3)
...
simpleAsinhX2<-function(value, cofactor = 5)
          {
            value <- value / cofactor
            value <- asinh(value)
            value <- asinh(value)
