...
cytof_exprsExtract <- function(fcsFile, 
    ...
    if (verbose) {
        ...
        fcs <- suppressWarnings(read.FCS(fcsFile, transformation = FALSE,truncate_max_range=F))
    }
    ...
    pd <- fcs@parameters@data
        ...
        exclude_channels<-colnames(fcs@exprs)
...
#  logicle数据转换
logicle<-flowCore::logicleTransform(w = 0.1, t = 4000, m = 4.5, a = 0)

#  log数据转换
logtrans<-flowCore::logTransform(logbase = 10,r=1,d=1)
