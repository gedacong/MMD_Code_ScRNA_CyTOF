...
#2021-1-8   修复stat.paired=F中默认组内样本数相等的问题
#2021-3-4   解决cluster别名的显示问题
...
draw_abundance_report<-function(cluster_stat,
  ...
  cat(paste0("Outputing Abundance Report\n"))
  ...
  copy_para_csv(output_dir,c("abundance_report_boxplot.csv","abundance_report_heatmap.csv","abundance_report_barplot.csv","abundance_report_instruction.pptx"))
  paralist_h<-get_para(output_dir,"abundance_report_heatmap.csv")
  eval(parse(text =paralist_h$items))
  paralist_bar<-get_para(output_dir,"abundance_report_barplot.csv")
  eval(parse(text =paralist_bar$items))
  paralist_boxplot<-get_para(output_dir,"abundance_report_boxplot.csv")
  ...
  cluster_count_summary   <-cluster_stat[["cluster_count_summary"]]
  cluster_percent_summary <-cluster_stat[["cluster_percent_summary"]]
  cluster_label_df<-cluster_stat[["cluster_label_df"]]
  ...
  { stop("heatmap_ctrl should be included in groups_to_show\n")
  ...
  if (alias_as_label==T){
     cluster_names<-as.character(cluster_id)
  }else{
...
#====   
  ...
  for(this_subgroup_cond in unique(cluster_percent_data[,subgroup_cond,drop=T])){
    ...
    for(this_major_cond in unique(single_subgroup_data[,major_cond,drop=T])){
      ...
      if(stat.paired==F){
        single_abundance_ctrl_one_row<-apply(single_abundance_ctrl[,cluster_names],2,median)
        for (row_i in 1:length(this_major_cond_id)) {
          single_abundance_ctrl[row_i,cluster_names]<-as.list(single_abundance_ctrl_one_row)#4.0
        }
      }
...
abundance_report<-function(hide_ctrl){      
      ...
      #=====================输出各个亚群的abundance boxplot================================
        ...
        #cluster_name=cluster_names[1]  
         #Plot_Data=cluster_percent_data
         #ylab="Percentage %"
         cluster_boxplot<-function(cluster_name,show_pvalues=T,Plot_Data,ylab){
                          ...
                          cluster_boxplot<- ggplot(Plot_Data,aes_string(x=major_cond,y="act_cluster",color=color_cond))+    
                                            geom_boxplot(outlier.shape= NA,lwd=border_size_b,width=box_width_b)+
                          ...
                          eval(parse(text = paste0("cluster_boxplot<-cluster_boxplot+",paralist_boxplot$themes)))
        ...
        # if(file_format=="PDF"){
          pdf(file=paste0("./",output_dir,"/","2.Cluster Abundance Boxplot",file_tag,".pdf"), width=boxplot_width/72*zoom_fig_width_b, height=boxplot_height/72*zoom_fig_height_b)
          ...
          # heatmap_color=heatmap1_color
            ...
            draw_heatmap<-function(xdata,
                        ...
                        x.inv<-try(heatmap.2(cluster_heatmap_data,
                                             ...
                                             scale="none",
                                             key=T,keysize =keysize,key.title = paste0(key.title),
                                             trace="none",
                                             col=heatmap_color,
            ...
            pdf(file=paste0("./",output_dir,"/","1.Cluster Abundance Heatmap",file_tag,".pdf"), 
                width=max(nrow(cluster_percent_data)/5+4,8)*zoom_fig_width_h, 
                height=max(length(cluster_names)/4+3,8)*zoom_fig_height_h)
      ...
      #=====================输出abundance barplot=====================================================================================
            ...
            if(barplot_clustered==T){      
             ...
             # cluster_barplot_data_melt<-left_join(cluster_barplot_data_melt,Manual_Pop_Index,by="cluster")
             # cluster_barplot_data_melt$cluster<-cluster_barplot_data_melt$Pop_name
             # cluster_barplot_data_melt$Pop_name<-NULL}
             #cluster_barplot_data_melt$Short_name<-factor(cluster_barplot_data_melt$Short_name,levels =levels(cluster_barplot_data$Short_name),ordered = T )
                  #画出纵向barplot
             cluster_colors<-cluster_colorset(clustern)
             if (exists("Others_color_bar")){
               if (!is.null(cluster_label_df)){
               cluster_colors[1]<-Others_color_bar}
             }
            ...
            gblank<-ggplot()+
             ...
             cluster_histogram<-ggplot(data=cluster_barplot_data_melt,aes_string(x="Short_name",fill="cluster",y="cluster_percentage")) +
                                ...
                                scale_y_continuous(limits = c(y_limit_bar[1],y_limit_bar[2]),expand = c(0,0))+
             ...
             eval(parse(text = paste0("cluster_histogram<-cluster_histogram+",paralist_bar$themes)))
            ...
            cat("Outputing cluster barplot",file_tag,"...\n")
              ...
              if(barplot_direction=="v"){
                              pdf(file=paste0("./",output_dir,"/","Cluster Abundance Histogram.pdf"), 
                                  width=max(nrow(cluster_barplot_data)/5+4,10)*zoom_fig_width_bar, 
                                  height=max(length(cluster_names)/4+3,8)*zoom_fig_width_bar)
                              #pdf(file=paste0("./",output_dir,"/","Cluster Abundance Histogram.pdf"), width=max(nrow(cluster_barplot_data)/5+4,8), height=max(length(cluster_names)/4+3,8))
                                ...
                                if(!exists("width_ratio_v_bar"))  width_ratio_v_bar =c(4,1)
                                if(!exists("height_ratio_v_bar")) height_ratio_v_bar=c(1,annotation_length,4)
                                merge_plot1<-ggarrange(p,gblank,
                                                       annotation_bar,gblank,
                                                       cluster_histogram2,histo_legend,
                                                       ncol = 2,
                                                       widths  =width_ratio_v_bar,
                                                       heights =height_ratio_v_bar
                                                       )
              ...
              if(barplot_direction=="h"){
                            ...
                            cluster_histogram<-ggplot(data=cluster_barplot_data_melt,aes_string(x="Short_name",fill="cluster",y="cluster_percentage")) +
                                                coord_flip()+
                                                geom_bar(position="stack", stat="identity",width = width_bar)+
                                                #labs(subtitle=paste0("Cluster Percentage"))+
                                                mytheme+scale_fill_manual(values=cluster_colorset(clustern))+
                                                scale_y_continuous(limits = c(y_limit_bar[1],y_limit_bar[2]),expand = c(0, 0),position = "right")+
                            ...
                            pdf(file=paste0("./",output_dir,"/","3.Cluster Abundance Histogram",file_tag,".pdf"), 
                                height=max(nrow(cluster_barplot_data)/5+5,8)*zoom_fig_height_bar, 
                                width = max(length(cluster_names)/4+3,8)*zoom_fig_height_bar)
                                ...
                                if(!exists("width_ratio_h_bar"))  width_ratio_v_bar =c(1,annotation_length,4)
                                if(!exists("height_ratio_h_bar")) height_ratio_v_bar=c(5,1)
                                merge_plot1<-ggarrange(p,annotation_bar,cluster_histogram2,
                                                       gblank,gblank,histo_legend,
                                                       ncol = 3,
                                                       widths  =width_ratio_h_bar,
                                                       heights =height_ratio_v_bar
                                                       )
                                multiplot(merge_plot1)
                                dev.off()
...
#'@param fig_size           vector,规定图片的宽度和高度，例如c(10,5),宽度是10，高度是5 (单位：inch),与zoomfactor一起控制输出图片大小. 
...
draw_abundance_volcano<-function(cluster_stat,
                            ...
                            ylimit=NULL ,
                            dif.level=2,
                            output_dir=paste0("Abundance_volcano_plots_",cond1,"_vs_",cond2),
                            zoom_factor=1,
                            #如使用全局统计默认参数，以下参数无需设置
                            stat.paired=NULL,
                            stat.method=NULL,
  ...
  copy_para_csv(output_dir,c("abundance_volcano_volcanoplot.csv","abundance_volcano_barplot.csv","abundance_volcano_volcanoplot.pptx"))
  ...
  cluster_volcano_data$cluster<-row.names(cluster_volcano_data)
  ...
  # if(!is.null(Manual_Pop_Index)){
  #       cluster_volcano_data<-left_join(cluster_volcano_data,Manual_Pop_Index,by="cluster")
  # }else{
  #   cluster_volcano_data$Pop_name<-cluster_volcano_data$cluster
  #    }

  paralist1<-get_para(output_dir,"abundance_volcano_volcanoplot.csv")
  ...
  cluster_volcano<-ggplot(cluster_volcano_data)+
    ...
    geom_point(aes_string(x="log_ratio",y=p_para,size="cluster_percent_mean"),color="black",shape=1)+
    labs(y="-Log10(p_values)",title="Differential Abundant Clusters")+
    labs(x=paste0("Log2_Ratio","\n", cond1, " <- enriched -> ", cond2,"\n\n Cite from SPADEVizR"))+
    labs(subtitle=paste0(adjust_label,"p values of cluster percentage were calculated by ",if_paired," ",stat.method))+
    mytheme+
    expand_limits(x=xlimit1,y=ylimit1)+
    ...
    scale_x_continuous(breaks=c(seq(-1*ceiling(xuplimit),ceiling(xuplimit),by=x_breakstep1),x_threshold,-1*x_threshold),minor_breaks = NULL)+
    scale_y_continuous(breaks=c(seq(y_limit1[1],y_limit1[2],by=y_breakstep1),round(-log10(1-conf.level),digits=1)),minor_breaks = NULL)+
    theme(plot.margin=unit(c(1,1,1,1), unit = "cm"))+
    theme(panel.background=element_rect(colour='black',size = 1 ))+
    theme(axis.line=element_line(colour='black',linetype = 1,size=0.3))+
    scale_color_manual(values =colorRampPalette(dot_colorset1)(2))+
    scale_radius(range=dot_radius1)+
    geom_text_repel(aes_string(x="log_ratio",y=p_para),label=cluster_volcano_data$cluster,color=label_color1,size=label_size1,box.padding=0.25,point.padding=0.5)
  eval(parse(text = paste0("cluster_volcano<-cluster_volcano+",paralist1$themes)))
  ...
  if(nrow(FDR_table_data)>0)
      FDR_table_data[,c(1,2,3)]<-apply(FDR_table_data[,c(1,2,3)],2,round_3)
  ...
  FDR_pValue$cluster<-factor(FDR_pValue$cluster,levels=FDR_pValue$cluster,ordered=TRUE)
  # if(is.null(Manual_Pop_Index)){axis_x<-"cluster_num"}else{
  #   axis_x<-"Pop_name"
  # }
  
  paralist2<-get_para(output_dir,"abundance_volcano_barplot.csv")
  eval(parse(text =paralist2$items))
  
  FDR_pValue_plot<-ggplot(FDR_pValue)+
    geom_bar(aes_string(x="cluster",y="log_p_value",fill="FDR_rank"),stat="identity",width =bar_width2)+
    ...
    mytheme+
    expand_limits(y=y_limit2)+
    theme(panel.background=element_rect(colour='white',size = 1 ))+
    theme(axis.line.x=element_line(colour='black',linetype = NULL,size=0.3))+
    theme(axis.ticks.y=element_blank())+
    scale_fill_manual(values = bar_color2)+
    scale_y_continuous(breaks=c(seq(y_limit2[1],y_limit2[2],by=y_breakstep2)),minor_breaks = NULL)+
  ...
  pdf(file=paste0("./",output_dir,"/Cluster Abundance Volcano_",cond1," enriched ",cond2,"_volcano.pdf"), width=7*zoom_fig_width1, height=10*zoom_fig_height1)
  multiplot(cluster_volcano)
  #multiplot(FDR_table)
  dev.off()
  
  pdf(file=paste0("./",output_dir,"/Cluster Abundance Volcano_",cond1," enriched ",cond2,"_barplot.pdf"), width=7*zoom_fig_width2, height=10*zoom_fig_height2)
