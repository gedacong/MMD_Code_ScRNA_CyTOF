...
#2020-11-13   增加imcacsih函数，变matches为one_of
#2021-3-1     增加manual gating 模块功能，对tsneheatmap进行修改
#2021-3-1     增加downsample绘图的错误处理，输出图片的错误不会影响downsample的进行
...
for(meta_i in unique(x[,sample_col])){
  ...
  data_sampled<-x%>%
    ...
    dplyr::filter_at(vars(one_of(sample_col)),all_vars(.==meta_i)) %>%
    sample_n(cell_n)
  data_sampled
...
draw_figures<-function(){
      ...
      tryCatch(draw_figures(),
               error=function(e){cat("Figures output failed.\n")})

cat('Downsample Finshed.\n')
return(combined_data_sampled[,input_col_name])

}


get_cluster_alias<-function(cluster_name,
                            alias_csv_dir){

  alias_csv<-paste0(cluster_name,"_alias.csv")
  if (file.exists(paste0(alias_csv_dir,'/',alias_csv))){
    cluster_label_df<-read.csv(paste0(alias_csv_dir,"/",alias_csv),stringsAsFactors = F)
    cluster_label_df<-dplyr::arrange_at(cluster_label_df,cluster_name)
    cluster_label_df[which(cluster_label_df[,2,drop=T]==""),2]="Others"
    cluster_label_df<-cluster_label_df%>%arrange_at(cluster_name)
    clevels<-c("Others",unique(cluster_label_df[cluster_label_df[,2]!="Others",][,2,drop=T]))
    cluster_label_df[,2]<-factor(cluster_label_df[,2],levels = clevels,ordered = T)
    return(cluster_label_df)
    
  }else{
    stop(paste0(alias_csv," not found.\n"))
  }
}
...
#' @param alias_as_label         布尔型变量，决定是否将cluster的别名做为图中的label
#' @param output_dir             输出数据文件夹名称
#' @param major_cond             选择groups中一个列名做为展现差异的major_cond
#' @param cluster_barplot_data   此参数已经弃用，barplot会直接根据绘图所用的数据绘制
...
#' @param edge_colour            图中连线的颜色，默认“grey”
...
draw_tsne_figs<-function(
  ...
  copy_para_csv(output_dir,c("draw_tsne_figs_barplot.csv","draw_tsne_figs_dotplots.csv","draw_tsne_figs_csv_instruction.pptx"))
  paralist_d<-get_para(output_dir,"draw_tsne_figs_dotplots.csv",item_silent = T)
  eval(parse(text =paralist_d$items))
  paralist_b<-get_para(output_dir,"draw_tsne_figs_barplot.csv",item_silent = T)
  eval(parse(text =paralist_b$items))  
  ...
  if(alias_as_label){
       cluster_label_df<-get_cluster_alias(cluster_name,"./output/")
       combined_data_plot<-left_join(combined_data_plot,cluster_label_df,by=cluster_name)
       label_name<-colnames(cluster_label_df)[2]
  } else{
    label_name<-cluster_name
    clevels<- sort(unique(combined_data_plot[,label_name,drop=T]))
    combined_data_plot[,label_name]=factor(combined_data_plot[,label_name],levels = sort(unique(combined_data_plot[,label_name])),ordered = T)   
     }
  
  
  if(!is.null(cluster_barplot_data)){
    message("warning message: \'cluster_barplot_data\' is deprecated, barplot will directly generated based on ploting data.\n" )
  }
  ...
  cluster_index<-colnames(combined_data_plot)==label_name
  ...
  #generate df data for labels in plot
  combined_data_plot<-combined_data_plot %>%
    group_by_at(label_name)%>%
  ...
  centers<-centers[centers[,1,drop=T]!="Others",]
  ...
  cluster_abandance_stat<-combined_data_plot%>%
                           dplyr::group_by_at(label_name)%>%
                               dplyr::summarise(num=n())

  
  cluster_num<-length(unique(combined_data_plot[,label_name]))
  ...
  cluster_abandance_stat$fill=cluster_color(nrow(cluster_abandance_stat))[as.numeric(cluster_abandance_stat[,label_name,drop=T])]
  if (!is.null(Others_color)) {
      other_idx=cluster_abandance_stat[,label_name,drop=T]=='Others'
      cluster_abandance_stat[other_idx,'fill']=Others_color
      }

  cluster_abundance_barplot<-ggplot(cluster_abandance_stat)+
                               geom_bar(aes_string(x=label_name,y='percentage',fill=label_name),stat = "identity",width = width_b)+
                               scale_fill_manual(values = cluster_abandance_stat$fill)+
                               mytheme+
                               coord_flip()+#横向
                               scale_x_discrete(limits=rev(levels(cluster_abandance_stat[,label_name,drop=T])))+
                               theme(plot.margin = unit(c(1,1,1,1), "cm"))
  
  eval(parse(text = paste0("cluster_abundance_barplot<-cluster_abundance_barplot+",paralist_b$themes)))
  
  if(output_format=="pdf"){
    pdf(file=paste0("./",output_dir,"/",rdm_name,"_",label_name," barplot.pdf"),width = zoom_fig_width_b*300/72,height = zoom_fig_height_b*30/72*nrow(cluster_abandance_stat))}else if(output_format=="tiff"){
      tiff(filename = paste0("./",output_dir,"/","tSNE-",label_name," barplot.tiff"),width=zoom_fig_width_b*300,height=zoom_fig_height_b*30*nrow(cluster_abandance_stat))} else
  ...
  draw_single_tsne_fig<-function(dotcolor_para=NULL,
             ...
             if (is.null(dotcolors)) dotcolors=cluster_color(length(unique(combined_data_plot[,dotcolor_para])))
            #产生facet图背景的数据：
            #登高线图
            ...
            if(!exists("x_limit_d")) x_limit_d=c(min(combined_data_plot[,reduction_dm1])*1.1,max(combined_data_plot[,reduction_dm1])*1.1)
            if(!exists("y_limit_d")) y_limit_d=c(min(combined_data_plot[,reduction_dm2])*1.1,max(combined_data_plot[,reduction_dm2])*1.1)
            
            
            plot1<-plot1+
              geom_point(aes_string(colour=dotcolor_para),size=dot_size)+
              scale_x_continuous(limits=x_limit_d)+
              scale_y_continuous(limits=y_limit_d)+
              mytheme+
            ...
            eval(parse(text = paste0("plot1<-plot1+",paralist_d$themes)))
  ...
  cluster_dot_colors= cluster_abandance_stat$fill

  #1生成合并tSNE-Phenograph图像
  
  if(output_format=="pdf"){
    pdf(file=paste0("./",output_dir,"/","1_",rdm_name,"_",label_name," merge.pdf"),width =zoom_fig_width1_d* 1300/72,height =zoom_fig_height1_d*1200/72)}else if(output_format=="tiff"){
      tiff(filename = paste0("./",output_dir,"/","1_","tSNE-",label_name," merge.tiff"),width=zoom_fig_width1_d*1300,height=zoom_fig_height1_d*1200)} else
      { message(paste0("Error-Output format ",output_format," is not supported\n"))
        return(NULL)}
  
      plot1<-draw_single_tsne_fig(dotcolor_para=label_name,dotcolors = cluster_dot_colors)
      multiplot(plot1)
      dev.off()
      
  cat(paste0("Exported: ",rdm_name,"_",label_name," merge.",output_format,"\n"))
  

  #2生成合并tSNE-文件图像

  if(output_format=="pdf"){
    pdf(file=paste0("./",output_dir,"/","3_",rdm_name,"_","Files"," merge.pdf"),width =zoom_fig_width3_d* 1300/72*(1+nrow(groups)/110),height = zoom_fig_height3_d*1200/72)}else if(output_format=="tiff"){
    tiff(filename = paste0("./",output_dir,"/","3_",rdm_name,"_","Files"," merge.tiff"),width=zoom_fig_width3_d*1300*(1+nrow(groups)/110),height=zoom_fig_height3_d*1200)} else{
      message(paste0("Error-Output format ",output_format," is not supported\n"))
    return(NULL)}
  ...
  if(output_format=="pdf"){
    pdf(file=paste0("./",output_dir,"/","4_",rdm_name,"_",label_name,"(sample name).pdf"),width =zoom_fig_width4_d*1200*nfacet_col/72,height =zoom_fig_height4_d*1200*nfacet_row/72)} else if(output_format=="tiff"){
    tiff(filename = paste0("./",output_dir,"/","4_",rdm_name,"-",label_name,"(sample name).tiff"),width = zoom_fig_width4_d*1200*nfacet_col,height =zoom_fig_height4_d*1200*nfacet_row/72)}else{
      message(paste0("Error-Output format ",output_format," is not supported\n"))
      return(NULL)}
  
  plot1<-draw_single_tsne_fig(dotcolor_para=label_name,
                              dotcolors=cluster_dot_colors,
                              facet_para = "Short_name",facet_ncol = ceiling(sqrt(nrow(groups))))

  multiplot(plot1)
  dev.off()
  cat(paste0("Exported: ",rdm_name,"_",label_name,"(sample name).",output_format,"\n"))
  ...
  if(output_format=="pdf"){
    pdf(file=paste0("./",output_dir,"/","2_",rdm_name,"_",label_name,"(",major_cond,").pdf"),width=zoom_fig_width2_d*1200*major_cond_n/72,height=zoom_fig_height2_d*1200/72)}else if(output_format=="tiff"){
    tiff(filename = paste0("./",output_dir,"/","2_",rdm_name,"_",label_name,"(",major_cond,").tiff"),width=zoom_fig_width2_d*1200*major_cond_n,height=zoom_fig_height2_d*1200)}  else{
      message(paste0("Error-Output format ",output_format," is not supported\n"))
    return(NULL)}
  
  plot1<-draw_single_tsne_fig(dotcolor_para=label_name,
                              dotcolors=cluster_dot_colors,
                              facet_para = major_cond,facet_ncol = major_cond_n)
  multiplot(plot1)
  dev.off()
...
#' @param groups                 实验组的Metadata信息,其中必需包含列名：”Short_name“
#' @param cluster_color          tSNE图的亚群的配色方案，默认dif_seq_rainbow
#' @param label_name           用过分析的cluster名称
#' @param output_dir             输出数据文件夹名称
#' @param major_cond             选择groups中一个列名做为展现差异的major_cond
...
#' @param show_axis              是否显示坐标轴，默认是TRUE
#' @param upperpct,lowerpct      分别指去除极值的百分比的上下限，默认是0.95和0.05
#' @param dot_size               图中点的大小，默认是4
#' @param dot_colours            设置colorbar的颜色，默认为jet color函数

#' @param show_contour           布尔型变量，决定是否显示等高线背景
#' @param edge_colour            连线的颜色
#' @param edge_line_size         连线的宽度      
#' @param edge_layer             连线的图层位置，"back"和"front"，决定连线图层在散点图的后面或前面
#' @param contour_line_size      设置等高线的宽度，默认0.5
#' @param contour_bins           设置等高线的层数，默认25
...
draw_tsne_heatmaps<-function(combined_data_plot,
                             ...
                             upperpct=0.95, #去除较高极值的百分比
                             lowerpct=0.05, #去除较低极值的百分比
  ...
  output_dir<-paste0(output_dir," ",trans_method)
  
  if (!dir.exists(paste0("./",output_dir))) {
    dir.create(paste0("./",output_dir))
  }
  
  copy_para_csv(output_dir,c("draw_tsne_heatmap.csv","cluster_heatmap_instruction.pptx"))
  paralist_d<-get_para(output_dir,"draw_tsne_heatmap.csv",item_silent = T)
  eval(parse(text =paralist_d$items))
  ...
  try(trans_fun<-get(trans_method),silent = T)     
  if(trans_method=="CytofAsinh") trans_fun=CytofAsinh
  ...
  combined_data_plot[,heatmap_tsne_markers]<-apply(combined_data_plot[,heatmap_tsne_markers,drop=F],
                                                   MARGIN = 2,
                                                   remove_extremum,uppct=upperpct,lowpct=lowerpct)
  ...
  if(!exists("x_limit_d")){
     x_limit_d<-c(min(combined_data_plot[,reduction_dm1])*1.1,max(combined_data_plot[,reduction_dm1])*1.1)}
  if(!exists("y_limit_d")){
    y_limit_d<-c(min(combined_data_plot[,reduction_dm2])*1.1,max(combined_data_plot[,reduction_dm2])*1.1)}
  ...
  single_heatmap_tsne<-function(marker_name){
    ...
    if((!is.null(edge_data)) & (edge_layer=="back")){
      single_heatmap<-single_heatmap+geom_line(data=edge_data_major_cond,aes_string(x=reduction_dm1,y=reduction_dm2,group="edge_id"),color=edge_colour,size=edge_line_size)
    ...
    eval(parse(text = paste0("single_heatmap<-single_heatmap+",paralist_d$themes)))
  ...
  if(single_file==FALSE){
    ...
    for(figure_i in 1:n_figure){
      ...
      if(output_format=="tiff"){
        jpeg(filename = paste0("./",output_dir,"/","Heatmap ",heatmap_tsne_markers[figure_i],".tiff"),width=zoom_fig_width_d*1000*fig_cond_n,height=zoom_fig_height_d*1000)}
      else if(output_format=="pdf"){
        pdf(file=paste0("./",output_dir,"/","Heatmap ",heatmap_tsne_markers[figure_i],".pdf"),width=zoom_fig_width_d*1000*fig_cond_n/72,height=zoom_fig_height_d*1000/72)}
      else{
        message(paste0("Error-Output format ",output_format," is not supported\n"))
        return(NULL)}
    ...
    if(output_format=="tiff"){
      jpeg(filename = paste0("./",output_dir,"/","scatter-heatmap.tiff"),height=zoom_fig_width_d*1000*fig_row,width=zoom_fig_height_d*1000*fig_col*fig_cond_n)}
    else if(output_format=="pdf"){
      jpeg(filename = paste0("./",output_dir,"/","scatter-heatmap.pdf"),height=zoom_fig_width_d*1000*fig_row/72,width=zoom_fig_height_d*1000*fig_col*fig_cond_n/72)}  
    else{
      message(paste0("Error-Output format ",output_format," is not supported\n"))
      return(NULL)}
